...
// One question in. Independent thinking, open debate, verified evidence,
// a weighted ballot — and a single verdict with the dissent preserved.
// Every seat in this build is powered by Claude; the provider layer is pluggable by design.
...
/* ================= THE ROSTER ================= */
/* The Expert Registry: the pool the Router selects from - including the
   Chair, Devil's Advocate, and Fact Checker, who are ordinary entries here
   like anyone else. Every seat is dynamic; none are permanently reserved.
   To add a new expert, append one object below - nothing else in the app
   needs to change. `h` (hue) is optional and auto-derived from `id` if
   omitted. `pref` is an optional preferred-provider hint used by the
   Router when that provider is available; otherwise the seat falls back
   to whatever provider you've selected as default. */
const EXPERT_REGISTRY = [
  // ---- process roles (routable/pinnable like any other expert now) ----
  ...
  { id: "devil", n: "Ash", r: "Devil's Advocate", m: "AS", h: 6, exp: "Adversarial review, failure modes, challenging groupthink and consensus, steelmanning the opposite case", bias: "Distrusts consensus by default", style: "Sharp, surgical, relentless", domain: "process", pref: "groq" },
  { id: "fact", n: "Vera", r: "Fact Checker", m: "VE", h: 262, exp: "Verification, sourcing, base rates, checking claims and evidence", bias: "Trusts evidence over eloquence", style: "Neutral, precise", domain: "process" },
  // ---- core cohort (original 19, unchanged aside from a domain tag) ----
  { id: "ceo", n: "Sloane", r: "CEO", m: "SL", h: 30, exp: "Strategy, focus, resource allocation", bias: "Speed and market position over perfection", style: "Decisive, big-picture", domain: "business" },
  { id: "cto", n: "Tanaka", r: "CTO", m: "TA", h: 200, exp: "Systems architecture, feasibility, technical debt", bias: "Skeptical of hype; guards maintainability", style: "Grounded, exacting", domain: "technology" },
  { id: "investor", n: "Marks", r: "Investor", m: "MK", h: 48, exp: "Capital allocation, unit economics, exits", bias: "Risk-adjusted returns; hates unpriced risk", style: "Blunt, numbers-first", domain: "finance" },
  { id: "lawyer", n: "Okafor", r: "Counsel", m: "OK", h: 280, exp: "Contracts, liability, compliance", bias: "Assumes the worst case; protects downside", style: "Careful, conditional", domain: "law" },
  { id: "scientist", n: "Petrova", r: "Research Scientist", m: "PE", h: 190, exp: "Research methods, causal inference", bias: "Demands evidence; allergic to anecdote", style: "Methodical, probabilistic", domain: "science" },
  { id: "economist", n: "Iyer", r: "Economist", m: "IY", h: 165, exp: "Incentives, markets, second-order effects", bias: "Follows incentives, not intentions", style: "Analytical, contrarian", domain: "economics" },
  { id: "psych", n: "Laurent", r: "Psychologist", m: "LA", h: 330, exp: "Behavior, motivation, cognitive bias", bias: "Centers the human factor", style: "Empathic, probing", domain: "psychology" },
  { id: "pm", n: "Reyes", r: "Product Lead", m: "RE", h: 210, exp: "User needs, prioritization, scope", bias: "Cuts scope; ships value early", style: "Pragmatic, crisp", domain: "business" },
  { id: "growth", n: "Kim", r: "Growth Strategist", m: "KI", h: 145, exp: "Distribution, funnels, retention", bias: "Distribution beats product polish", style: "Energetic, experimental", domain: "business" },
  { id: "ux", n: "Andersen", r: "Design Lead", m: "AN", h: 300, exp: "Interaction design, clarity, craft", bias: "Fights complexity on the user's behalf", style: "Visual, opinionated", domain: "design" },
  { id: "data", n: "Chen", r: "Data Scientist", m: "CH", h: 220, exp: "Metrics, experiments, models", bias: "If it is unmeasured, it is a guess", style: "Quantitative, calm", domain: "technology" },
  ...
  { id: "psychiatrist", n: "Lindqvist", r: "Psychiatrist", m: "LI", exp: "Mental health treatment, medication management, risk assessment", bias: "Weighs quality of life alongside symptom reduction", style: "Careful, non-judgmental, precise", domain: "medicine", pref: "local" },
  ...
  { id: "privacy", n: "Renner", r: "Privacy & Compliance Officer", m: "RN", exp: "Data protection law, regulatory exposure, audits", bias: "Assumes regulators read the worst-case interpretation", style: "Careful, documentation-minded", domain: "cybersecurity", pref: "local" },
  ...
  // ---- psychology ----
  { id: "clinpsych", n: "Moreau", r: "Clinical Psychologist", m: "MO", exp: "Therapy modalities, diagnosis, treatment planning", bias: "Weighs the person's lived experience over the checklist", style: "Warm, careful, boundaried", domain: "psychology", pref: "local" },
...
const EX = (id) => EXPERTS.find((e) => e.id === id) || { id, n: id || "?", r: "Guest", m: String(id || "??").slice(0, 2).toUpperCase(), h: hueFor(String(id || "??")), exp: "", bias: "", style: "" };
const PINNABLE = EXPERTS;
const EXPERT_SEATS = 6;

/* ================= EXPERT ROUTER ================= */
/* Cheap lexical pre-filter over the full registry, so the routing LLM call
   gets a focused, relevant shortlist instead of 100+ lines every question.
   Manually pinned experts are always force-included. Adding a new persona
   to EXPERT_REGISTRY needs no change here - it is scored generically. */
...
function shortlistExperts(question, pinIds, n) {
  ...
  const forced = (pinIds || []).map((id) => EXPERT_REGISTRY.find((e) => e.id === id)).filter(Boolean);
  const seen = new Set();
  const out = [];
...
const PROVIDERS = [
  { id: "anthropic", label: "Claude (built-in)", needsKey: false, needsEndpoint: false,
    defaultModel: "claude-sonnet-4-6", liveSearch: true,
    hint: "Runs through Adep's built-in Claude access - no key needed, and this is the only path with live web research wired up." },
  { id: "openai", label: "OpenAI", needsKey: true, needsEndpoint: false,
    defaultModel: "gpt-5.6", liveSearch: false,
    hint: "Paste a key from platform.openai.com. Kept in this browser tab only - never saved, never sent anywhere but OpenAI." },
  { id: "gemini", label: "Gemini", needsKey: true, needsEndpoint: false,
    defaultModel: "gemini-3.5-flash", liveSearch: false,
    hint: "Paste a key from Google AI Studio. Google's API commonly blocks direct browser requests (CORS) - if it fails immediately, try the same model via OpenRouter instead." },
  { id: "groq", label: "Groq", needsKey: true, needsEndpoint: false,
    defaultModel: "openai/gpt-oss-120b", liveSearch: false,
    hint: "Paste a key from console.groq.com. Fast open-weight inference." },
  { id: "openrouter", label: "OpenRouter", needsKey: true, needsEndpoint: false,
    defaultModel: "openai/gpt-4o-mini", liveSearch: false,
    hint: "Paste a key from openrouter.ai. Model id uses OpenRouter's provider/model format, e.g. anthropic/claude-3.7-sonnet." },
  { id: "local", label: "Ollama / local", needsKey: false, needsEndpoint: true,
    defaultModel: "llama3.1", defaultEndpoint: "http://localhost:11434/v1/chat/completions", liveSearch: false,
    hint: "Works with Ollama, LM Studio, llama.cpp server or vLLM's OpenAI-compatible endpoint. Browsers often block a hosted page from reaching your localhost server - treat this one as experimental." },
];
const PROV = (id) => PROVIDERS.find((p) => p.id === id) || PROVIDERS[0];

function providerError(providerId, model, status, message) {
  let kind = "unknown";
  if (status === 429) kind = "rate_limit";
  else if (status === 401 || status === 403) kind = "auth";
  const e = new Error("[" + providerId + "/" + (model || "?") + "] " + (status ? "HTTP " + status + " - " : "") + (message || "Request failed"));
  e.kind = kind; e.provider = providerId;
  return e;
}

async function sendAnthropic(system, user, tools, cfg) {
  const body = { model: (cfg && cfg.model) || "claude-sonnet-4-6", max_tokens: 1000, system, messages: [{ role: "user", content: user }] };
  if (tools) body.tools = tools;
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body),
  });
  const data = await res.json().catch(() => ({}));
  if (data.error) throw providerError("anthropic", body.model, res.status, data.error.message || data.error.type);
  if (!res.ok) throw providerError("anthropic", body.model, res.status, "Request failed");
  return (data.content || []).map((x) => (x.type === "text" ? x.text : "")).filter(Boolean).join("\n");
}

async function sendOpenAICompatible(providerId, baseUrl, system, user, cfg) {
  const headers = { "Content-Type": "application/json" };
  if (cfg && cfg.key) headers.Authorization = "Bearer " + cfg.key;
  const res = await fetch(baseUrl, {
    method: "POST", headers,
    body: JSON.stringify({ model: cfg && cfg.model, messages: [{ role: "system", content: system }, { role: "user", content: user }], max_tokens: 1000 }),
  });
  const data = await res.json().catch(() => ({}));
  if (!res.ok) throw providerError(providerId, cfg && cfg.model, res.status, (data.error && data.error.message) || res.statusText);
  const text = data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content;
  if (!text) throw providerError(providerId, cfg && cfg.model, res.status, "Empty response from provider");
  return text;
}

async function sendGemini(system, user, cfg) {
  const model = (cfg && cfg.model) || "gemini-3.5-flash";
  const url = "https://generativelanguage.googleapis.com/v1beta/models/" + model + ":generateContent?key=" + encodeURIComponent((cfg && cfg.key) || "");
  const res = await fetch(url, {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ systemInstruction: { parts: [{ text: system }] }, contents: [{ role: "user", parts: [{ text: user }] }] }),
  });
  const data = await res.json().catch(() => ({}));
  if (!res.ok) throw providerError("gemini", model, res.status, (data.error && data.error.message) || res.statusText);
  const cand = data.candidates && data.candidates[0];
  const text = cand && cand.content && cand.content.parts && cand.content.parts.map((p) => p.text || "").join("");
  if (!text) throw providerError("gemini", model, res.status, "Empty response from provider");
  return text;
}

async function providerSend(system, user, tools, cfg) {
  const id = (cfg && cfg.id) || "anthropic";
  try {
    if (id === "anthropic") return await sendAnthropic(system, user, tools, cfg);
    if (id === "gemini") return await sendGemini(system, user, cfg);
    if (id === "openai") return await sendOpenAICompatible("openai", "https://api.openai.com/v1/chat/completions", system, user, cfg);
    if (id === "groq") return await sendOpenAICompatible("groq", "https://api.groq.com/openai/v1/chat/completions", system, user, cfg);
    if (id === "openrouter") return await sendOpenAICompatible("openrouter", "https://openrouter.ai/api/v1/chat/completions", system, user, cfg);
    if (id === "local") return await sendOpenAICompatible("local", (cfg && cfg.endpoint) || "http://localhost:11434/v1/chat/completions", system, user, cfg);
    throw new Error("Unknown provider: " + id);
  } catch (e) {
    if (e && e.kind) throw e;
    const ne = new Error("[" + id + "] Could not reach the provider from this page - likely a network or CORS restriction. " + ((e && e.message) || ""));
    ne.kind = "network"; ne.provider = id;
    throw ne;
  }
}

async function askJSON(system, user, tools, cfg) {
...
function useCouncil() {
  const [phase, setPhase] = useState("idle"); // idle | running | report | error
  const [stageIdx, setStageIdx] = useState(-1);
  const [qText, setQText] = useState("");
  const [modeId, setModeId] = useState("quick");
  const [seats, setSeatsState] = useState([]);
  ...
  const syncSeats = () => setSeatsState(seatsRef.current.map((s) => ({ ...s })));
  const seat = (id, patch) => {
  ...
  const link = (from, to, k) => setLinks((ls) => [...ls.slice(-2), { id: uid(), from, to, k }]);
  const mark = (label) => { setLinks([]); push({ t: "stageMark", label }); };
  const bail = () => cancelRef.current;

  async function convene(question, mId, pins, research, providerCfg, resolveExpertProvider) {
    ...
    try {
      /* Stage 1 — Understanding (Expert Router: shortlist, then the LLM picks) */
      setStageIdx(0); setBusy("Reading the question");
      const idx = (await store.get("adep:index")) || { meetings: [] };
      const recent = idx.meetings.slice(-2).map((m) => '- "' + m.q + '" decided: ' + (m.rec || "-") + " (confidence " + (m.conf == null ? "?" : m.conf) + "%)").join("\n");
      ...
      const a = await askJSON(
        ...
        '\nSelect the ' + EXPERT_SEATS + ' most relevant experts for this specific question - anyone in the registry is eligible, including the Chair, Devil\'s Advocate, and Fact Checker where genuinely relevant; favor a precise specialist over a generalist when one fits.' +
      ...
      /* Stage 2 — Formation: the Router's picks (plus pins) ARE the six seats, full stop - no role is ever added automatically */
      setStageIdx(1); setBusy("Seating the council");
      let ids = (a.expertIds || []).filter((id) => EXPERT_REGISTRY.some((e) => e.id === id));
      ...
      seatIds.forEach((id) => {
        const meta = EX(id);
        const resolved = meta.pref ? resolveExpertProvider(meta.pref) : null;
        seatProviders[id] = resolved || providerCfg;
      });
      recRef.current.seatIds = seatIds;
      recRef.current.seatProviders = Object.fromEntries(Object.entries(seatProviders).map(([k, v]) => [k, v.id]));
      ...
      /* Stage 3 — Independent thinking: every seated member participates fully - no seat is exempt from thinking, presenting, debating, or voting */
      ...
      for (let round = 1; round <= mode.rounds; round++) {
        ...
        const msgs = (d.messages || []).filter((m) => seatIds.includes(m.s));
        for (const m of msgs) {
          if (bail()) return;
          setActive(m.s);
          ...
          const s = seatsRef.current.find((x) => x.id === m.s);
      ...
      /* Stage 6 — Evidence review: the check still happens every session; Vera performs it herself only if she is one of the six this round */
      ...
      for (const ch of (ev.checks || []).slice(0, 5)) {
        ...
        if (ch.verdict === "disputed" && ch.by) {
          const s = seatsRef.current.find((x) => x.id === ch.by);
      ...
      const v = await askJSON(
        ...
        "\nEvidence checks:\n" + ((ev.checks || []).map((c) => c.verdict + ": " + c.claim).join("\n") || "none") +
      ...
      const ballots = (v.ballots || []).filter((b) => voters.includes(b.s));
      const totals = {};
      opts.forEach((o) => { totals[o.id] = 0; });
      ballots.forEach((b) => {
      ...
      setActive(null);
      const optPack = { options: opts, totals, winner, unanimous: ballots.length > 0 && ballots.every((b) => b.vote === ballots[0].vote) };
      ...
      const winLabel = (opts.find((o) => o.id === winner) || {}).label || "-";
      const r = await askJSON(
        ENGINE_SYS,
        'Question: """' + question + '"""\nWinning option: ' + winLabel + " (weighted vote " + (totals[winner] || 0) + ")" +
        "\nAll options: " + opts.map((o) => o.id + ": " + o.label + " (" + (totals[o.id] || 0) + ")").join(" | ") +
        "\nBallots: " + ballots.map((b) => b.s + " voted " + b.vote + " at " + clamp(b.conf, 1, 100) + "%" + (b.changed ? " (revised)" : "")).join(", ") +
        "\nPositions:\n" + ser +
        "\nEvidence:\n" + ((ev.checks || []).map((c) => c.verdict + ": " + c.claim + " - " + c.note).join("\n") || "none") +
      ...
      recRef.current.confs = Object.fromEntries(seatsRef.current.map((s) => [s.id, s.conf]));
      setPhase("report"); setBusy("");

      /* Persist the meeting */
      const rec = recRef.current;
      await store.set("adep:meeting:" + rec.id, rec);
      ...
      await store.set("adep:index", idx);
    } catch (err) {
      console.error(err);
  ...
  async function followUp(text, providerCfg) {
    ...
    push({ t: "user", text });
    try {
      const seatIds = (recRef.current && recRef.current.seatIds.length ? recRef.current.seatIds : seatsRef.current.map((s) => s.id));
      ...
      setReport(r2);
      if (recRef.current) { recRef.current.report = r2; await store.set("adep:meeting:" + recRef.current.id, recRef.current); }
    } catch (e) {
...
function Ghost({ children, onClick, style, label }) {
  return (
...
/* ================= THE COUNCIL TABLE ================= */
function SeatRing({ seats, active, links, title, sub }) {
  ...
  return (
    <svg viewBox={"0 0 " + W + " " + H} style={{ width: "100%", height: "auto", display: "block" }} role="img" aria-label="Council table">
      ...
      {links.map((l) => (pos[l.from] && pos[l.to]) ? (
      ...
      {seats.map((s) => {
        const e = EX(s.id); const p = pos[s.id]; const r = 24; const CIRC = 2 * Math.PI * (r + 5);
        return (
...
function OptionsPanel({ options, totals, winner, unanimous }) {
  const max = Math.max(...options.map((o) => totals[o.id] || 0), 1);
  return (
    <Card accent={C.brass}>
      <Cap style={{ color: C.brass }}>{"Weighted ballot" + (unanimous ? " · unanimous" : "")}</Cap>
      {options.map((o) => {
        const v = totals[o.id] || 0; const w = Math.round((v / max) * 100);
        return (
          <div key={o.id} style={{ margin: "12px 0 2px" }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
              ...
              <span style={{ fontFamily: MONO, fontSize: 11, color: C.dim }}>{v}</span>
...
function ReportView({ r, options, onFollow }) {
  if (!r) return null;
  const minor = r.minority && r.minority.holder ? EX(r.minority.holder) : null;
  const winLabel = options ? ((options.options.find((o) => o.id === options.winner) || {}).label || null) : null;
  const sev = { high: C.bad, med: C.warn, low: C.dim };
  return (
...
/* ================= HOME ================= */
function HomeView({ onConvene, meetings, onHistory, onOpen, providers }) {
  ...
  const togglePin = (id) => setPins((p) => p.includes(id) ? p.filter((x) => x !== id) : (p.length >= EXPERT_SEATS ? p : [...p, id]));
  ...
  return (
    <div className="fu">
      ...
      <div style={{ background: C.bg1, border: "1px solid " + C.line, borderRadius: 16, padding: 14 }}>
        ...
        {research && providers.providerId !== "anthropic" ? (
          <div style={{ fontFamily: SANS, fontSize: 11, color: C.faint, margin: "2px 2px 10px" }}>
            Live search runs on the built-in Claude provider regardless of this toggle; {providers.meta.label} verifies from training knowledge instead.
          </div>
            ...
            <div className="rowscroll" style={{ display: "flex", gap: 7, overflowX: "auto", paddingBottom: 4 }}>
              {PROVIDERS.map((p) => (
                <button key={p.id} onClick={() => providers.setProviderId(p.id)} style={{ flex: "none", fontFamily: SANS, fontSize: 12, fontWeight: 600,
                  color: p.id === providers.providerId ? C.brassHi : C.dim, border: "1px solid " + (p.id === providers.providerId ? C.brass : C.line),
                  background: p.id === providers.providerId ? C.brassDim : "transparent", borderRadius: 999, padding: "7px 13px" }}>{p.label}</button>
              ))}
            </div>
            <div style={{ fontFamily: SANS, fontSize: 11, color: C.faint, margin: "8px 2px" }}>{providers.meta.hint}</div>
            {providers.meta.needsKey ? (
              <input type="password" value={providers.cfg.key} onChange={(e) => providers.setKey(providers.providerId, e.target.value)}
                placeholder={"API key for " + providers.meta.label} aria-label={"API key for " + providers.meta.label}
                style={{ width: "100%", background: C.bg2, border: "1px solid " + C.line, borderRadius: 9, padding: "9px 11px",
                  color: C.ink, fontFamily: MONO, fontSize: 12.5, marginBottom: 8 }} />
            ) : null}
            {providers.meta.needsEndpoint ? (
              <input value={providers.cfg.endpoint} onChange={(e) => providers.setEndpoint(providers.providerId, e.target.value)}
                placeholder="Endpoint URL" aria-label="Provider endpoint URL"
                style={{ width: "100%", background: C.bg2, border: "1px solid " + C.line, borderRadius: 9, padding: "9px 11px",
                  color: C.ink, fontFamily: MONO, fontSize: 12.5, marginBottom: 8 }} />
            ) : null}
            <input value={providers.cfg.model} onChange={(e) => providers.setModel(providers.providerId, e.target.value)}
              placeholder="Model id" aria-label="Model id"
              style={{ width: "100%", background: C.bg2, border: "1px solid " + C.line, borderRadius: 9, padding: "9px 11px",
                color: C.ink, fontFamily: MONO, fontSize: 12.5 }} />
            {!providers.ready ? (
              <div style={{ fontFamily: SANS, fontSize: 11, color: C.warn, marginTop: 8 }}>Add a key above to convene using {providers.meta.label}.</div>
            ) : null}
            <div style={{ fontFamily: SANS, fontSize: 11, color: C.faint, marginTop: 10 }}>
              Available for the Router to assign per-expert: {providers.availableIds.map((id) => PROV(id).label).join(", ")}. Configure another provider above to add it to the pool.
            </div>
          </div>
        ) : null}
        ...
        {showPins ? (
          <div style={{ marginBottom: 14 }}>
            <input value={expQuery} onChange={(e) => setExpQuery(e.target.value)}
            ...
            <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
              {(() => {
                ...
                const pool = q
                  ? PINNABLE.filter((e) => (e.id + " " + e.n + " " + e.r + " " + e.domain + " " + e.exp).toLowerCase().includes(q))
                  : PINNABLE.slice(0, 18);
      ...
      <div style={{ fontFamily: SANS, fontSize: 11, color: C.faint, textAlign: "center", marginTop: 30, lineHeight: 1.6 }}>
        Claude runs built-in, no key needed.<br />Switch to OpenAI, Gemini, Groq, OpenRouter, or a local model above with your own key.
      </div>
    </div>
  );
}
...
function HistoryView({ meetings, onBack, onOpen }) {
  const count = meetings.length;
  const avg = count ? Math.round(meetings.reduce((a, m) => a + (m.conf || 0), 0) / count) : 0;
  const unan = count ? Math.round((meetings.filter((m) => m.unanimous).length / count) * 100) : 0;
  const tally = {};
  meetings.forEach((m) => (m.seatIds || []).forEach((id) => {
...
function MeetingView({ id, onBack }) {
  const [rec, setRec] = useState(null);
  const [tab, setTab] = useState("report");
  useEffect(() => { (async () => setRec(await store.get("adep:meeting:" + id)))(); }, [id]);
  return (
    <div className="fu">
...
/* ================= LIVE SESSION ================= */
function SessionView({ council, onHome, providerCfg }) {
  ...
  const sendFollow = (text) => { const v = (text != null ? text : fq).trim(); if (!v) return; setFq(""); setTab("transcript"); followUp(v, providerCfg); };
...
function useProviderConfig() {
  ...
  useEffect(() => {
    (async () => {
      const saved = await store.get("adep:providerPrefs");
      if (saved) {
        setProviderId(saved.providerId || "anthropic");
        setModels(saved.models || {});
        setEndpoints(saved.endpoints || {});
      }
      setLoaded(true);
    })();
  }, []);
  useEffect(() => {
    if (!loaded) return;
    store.set("adep:providerPrefs", { providerId, models, endpoints });
  }, [loaded, providerId, models, endpoints]);

  const setModel = (id, m) => setModels((s) => ({ ...s, [id]: m }));
  const setEndpoint = (id, e) => setEndpoints((s) => ({ ...s, [id]: e }));
  const setKey = (id, k) => setKeys((s) => ({ ...s, [id]: k }));

  const isAvailable = (id) => { const m = PROV(id); return m.needsKey ? !!keys[id] : true; };
  const resolveFor = (id) => {
    if (!id || !isAvailable(id)) return null;
    const m = PROV(id);
    return { id, model: models[id] || m.defaultModel, endpoint: endpoints[id] || m.defaultEndpoint, key: keys[id] || "" };
  };
  const availableIds = PROVIDERS.filter((p) => isAvailable(p.id)).map((p) => p.id);

  const meta = PROV(providerId);
  const cfg = {
    id: providerId,
    model: models[providerId] || meta.defaultModel,
    endpoint: endpoints[providerId] || meta.defaultEndpoint,
    key: keys[providerId] || "",
  };
  const ready = !meta.needsKey || !!cfg.key;

  return { providerId, setProviderId, setModel, setEndpoint, setKey, cfg, meta, ready, resolveFor, availableIds };
}
...
export default function Adep() {
  ...
  useEffect(() => {
    (async () => {
      const i = await store.get("adep:index");
      setMeetings(((i && i.meetings) || []).slice().reverse());
    })();
  }, [view, council.phase]);

  const start = (q, mId, pins, research) => { setView("session"); council.convene(q, mId, pins, research, providers.cfg, providers.resolveFor); };
  ...
  return (
    <div style={{ minHeight: "100vh", color: C.ink, fontFamily: SANS,
      ...
      <div style={{ maxWidth: 780, margin: "0 auto", padding: "20px 16px 70px" }}>
        {view === "home" ? (
          <HomeView onConvene={start} meetings={meetings} onHistory={() => setView("history")} onOpen={(id) => { setOpenId(id); setView("meeting"); }} providers={providers} />
        ) : null}
        {view === "session" ? <SessionView council={council} onHome={goHome} providerCfg={providers.cfg} /> : null}
        {view === "history" ? <HistoryView meetings={meetings} onBack={() => setView("home")} onOpen={(id) => { setOpenId(id); setView("meeting"); }} /> : null}
        {view === "meeting" ? <MeetingView id={openId} onBack={() => setView("history")} /> : null}
      </div>
    </div>
  );
}
